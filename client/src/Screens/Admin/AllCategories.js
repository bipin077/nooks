        ...
        <table class="table table-striped">
          ...
          { !isFetching && data.category.map((category, index) =>
            <tr key={index}>
              ...
              <td width="10%"><img src={ `/upload/category/${category.image}` } style={{"width" : "70%"}} class="rounded"  /></td>
              <td width="40%">{category.title}</td>
              <td width="10%">{ category.status ? "Active" : "Inactive" }</td>
              <td width="10%"><Link to={ `/admin/editCategory/${category._id}`}><i class="fas fa-edit mr-10"></i></Link></td>
              <td width="10%"><i class="fas fa-trash mr-10" onClick={()=>deleteCategory(category._id)}></i></td>
            </tr>
          )}
